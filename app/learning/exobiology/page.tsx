...
const sidebarSignals = exobiologyTopics.map((topic) => ({
  title: topic.title,
  signal: topic.signal,
}));
  ...
  return (
    <>
      <TerminalHeader
        ...
        statusLabel={`${exobiologyTopics.length} field guide${exobiologyTopics.length !== 1 ? "s" : ""} indexed`}
      />

      <BreadcrumbNav
        backHref="/learning"
        backLabel="Learning Resources"
        rightIcon="icarus-terminal-planet-atmosphere"
        rightLabel="MODULE - EXOBIOLOGY"
      />

      <Panel className="fx-chamfer fx-panel-scan mb-5 px-4 py-4 md:px-6 md:py-5">
        <div className="flex items-start gap-3">
          <i className="icarus-terminal-planet-atmosphere text-glow__blue mt-0.5 text-2xl" />
          <div>
