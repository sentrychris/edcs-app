...
const BootSequence: FunctionComponent = () => {
  ...
  return (
    ...
    >
      ...
      {/* HUD chrome — top */}
      <div className="pointer-events-none absolute left-5 top-5 space-y-0.5 text-[0.45rem] uppercase tracking-[0.45em] text-sky-900/50">
        <div>ED:CS / BUILD 4.7.2</div>
        <div>PILOTS FEDERATION AUTH</div>
      </div>
      <div className="pointer-events-none absolute right-5 top-5 space-y-0.5 text-right text-[0.45rem] uppercase tracking-[0.45em] text-sky-900/50">
        <div>STARDATE 3309.04.10</div>
        <div>SECURE CHANNEL</div>
      </div>

      {/* HUD chrome — bottom */}
      <div className="pointer-events-none absolute bottom-5 left-5 text-[0.42rem] uppercase tracking-[0.45em] text-sky-900/30">
        © FRONTIER DEVELOPMENTS / GALACTIC COMMS
      </div>
      <div className="pointer-events-none absolute bottom-5 right-5 text-[0.42rem] uppercase tracking-[0.45em] text-sky-900/30">
        ENCRYPTED ■ CHANNEL ACTIVE
      </div>

      {/* Horizontal rule lines */}
      <div className="pointer-events-none absolute inset-x-12 top-14 h-px bg-sky-900/20" />
      <div className="pointer-events-none absolute inset-x-12 bottom-14 h-px bg-sky-900/20" />

      {/* Vertical rule lines */}
      <div className="pointer-events-none absolute inset-y-14 left-14 w-px bg-sky-900/10" />
      <div className="pointer-events-none absolute inset-y-14 right-14 w-px bg-sky-900/10" />

      {/* Scanning beam */}
      <div
        className="pointer-events-none absolute inset-x-0 z-10 h-px"
        style={{
          background: "linear-gradient(90deg, transparent 0%, rgba(120,200,255,0.25) 20%, rgba(120,200,255,0.65) 50%, rgba(120,200,255,0.25) 80%, transparent 100%)",
      ...
      <div className="relative flex flex-col items-center justify-center">
        ...
        >
          ...
          <g style={{ transformOrigin: "200px 200px", animation: "boot-spin 45s linear infinite" }}>
            <circle cx="200" cy="200" r="188" stroke="rgba(120,200,255,0.1)" strokeWidth="0.5" />
            {TICKS.map(({ a, isMajor, isMid, r1, r2 }, i) => (
              <line
                key={i}
                x1={200 + r1 * Math.cos(a)} y1={200 + r1 * Math.sin(a)}
                x2={200 + r2 * Math.cos(a)} y2={200 + r2 * Math.sin(a)}
                stroke={`rgba(120,200,255,${isMajor ? 0.5 : isMid ? 0.28 : 0.14})`}
          ...
          {/* Inner static ring */}
          <circle cx="200" cy="200" r="116" stroke="rgba(120,200,255,0.1)" strokeWidth="0.75" />

          {/* Ambient inner glow ring */}
          <circle cx="200" cy="200" r="116" stroke="rgba(120,200,255,0.04)" strokeWidth="28" />

          {/* Crosshair arms */}
          <line x1="200" y1="8"   x2="200" y2="84"  stroke="rgba(120,200,255,0.2)" strokeWidth="0.75" />
          <line x1="200" y1="316" x2="200" y2="392" stroke="rgba(120,200,255,0.2)" strokeWidth="0.75" />
          <line x1="8"   y1="200" x2="84"  y2="200" stroke="rgba(120,200,255,0.2)" strokeWidth="0.75" />
          <line x1="316" y1="200" x2="392" y2="200" stroke="rgba(120,200,255,0.2)" strokeWidth="0.75" />
          ...
          {([[200, 8], [200, 392], [8, 200], [392, 200]] as [number, number][]).map(([x, y], i) => (
            <line key={i}
              ...
              stroke="rgba(120,200,255,0.45)" strokeWidth="1.5"
            />
          ))}
          ...
          {[0, 90, 180, 270].map((deg, i) => {
            ...
            return (
              <circle key={i}
                ...
                style={{ filter: "drop-shadow(0 0 4px rgba(120,200,255,0.9))" }}
              />
            );
          })}
        ...
        {phase === "logo" && (
          <div className="relative z-10 flex flex-col items-center">
            <div className="mb-2 text-[0.42rem] uppercase tracking-[0.6em] text-sky-800/55">
              FRONTIER DEVELOPMENTS / PILOTS FEDERATION
            </div>
            <h1
              className="text-glow__blue text-4xl font-bold uppercase tracking-[0.5em]"
              style={{ animation: "fx-data-flicker 0.7s ease forwards, 12s ease-in-out 1s infinite" }}
            >
              ED:CS
            </h1>
            <div className="mt-2 text-[0.48rem] uppercase tracking-[0.5em] text-sky-700/50">
        ...
        {showChecks && (
          <div className="relative z-10" style={{ width: "min(18rem, 82vw)" }}>
            <div className="mb-3 border-b border-sky-900/30 pb-2 text-center text-[0.42rem] uppercase tracking-[0.45em] text-sky-800/50">
            ...
            <div className="space-y-1.5">
              {CHECKS.slice(0, checks).map((check, i) => (
                <div key={i} className="flex items-center justify-between gap-4 text-[0.65rem] uppercase tracking-wider">
                  <span className="text-sky-700/70">&gt; {check}</span>
                  <span
            ...
            {showProgress && (
              <div className="mt-5">
                <div className="mb-1.5 flex items-center justify-between text-[0.48rem] uppercase tracking-widest">
                  <span className="text-sky-900/60">SYSTEM STATUS</span>
                  <span className="text-glow__blue tabular-nums">{progress}%</span>
                </div>
                <div className="relative h-[2px] overflow-hidden bg-sky-950/80">
                  <div
                    className="absolute inset-y-0 left-0"
                    style={{
                      width: `${progress}%`,
                      background: "rgb(80,180,245)",
                      boxShadow: "0 0 10px rgba(80,180,245,0.8), 0 0 24px rgba(80,180,245,0.35)",
            ...
            {phase === "complete" && (
              <div className="mt-6 text-center">
                <div
                  className="text-glow__blue text-sm font-bold uppercase tracking-[0.5em]"
                  style={{ animation: "fx-data-flicker 0.5s ease forwards, 8s ease-in-out 0.6s infinite" }}
                >
                  WELCOME, COMMANDER
                </div>
              </div>
