...
const Sidebar: FunctionComponent<Props> = ({ articles, user }) => {
  ...
  return (
    <aside
      className={`hidden shrink-0 flex-col overflow-x-hidden overflow-y-auto border-r border-sky-900/40 bg-black/50 backdrop-blur transition-[width] duration-200 lg:flex ${
        collapsed ? "w-14" : "w-64 xl:w-80"
      }`}
    >
      {/* ── Header ── */}
      <div className="flex shrink-0 items-center gap-3 border-b border-sky-900/40 px-4 py-4">
        <i className="icarus-terminal-logo text-glow__blue shrink-0 text-2xl"></i>
      ...
      <div className="mt-auto shrink-0 border-t border-sky-900/20 px-3 py-3">
        {collapsed ? (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-neutral-800" style={{ fontSize: "0.6rem" }}>
              <i className="icarus-terminal-route text-sky-500/20"></i>
              <span className="uppercase tracking-widest">SYS:EDCS-001 ■ BUILD:STABLE</span>
            </div>
            <div className="ml-3 flex shrink-0 items-center gap-2">
              <button
                onClick={() => setSettingsOpen(true)}
                className="text-neutral-700 transition-colors hover:text-sky-400"
                aria-label="Open settings"
                title="Settings"
              >
                <i className="icarus-terminal-settings text-sm"></i>
