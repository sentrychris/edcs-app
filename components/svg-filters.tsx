"use client";

import { type FunctionComponent, memo } from "react";

const SvgFilters: FunctionComponent = () => {
  return (
    <>
      <svg style={{ position: "absolute", height: 0, margin: 0, padding: 0, top: "-100px" }}>
        <defs>
          {/* Clip Path for Planet Icons */}
          <clipPath id="svg-clip-path__planet">
            <rect x="400" y="0" width="500" height="1000" />
          </clipPath>

          {/* Gradients */}
          <linearGradient id="svg-gradient__unknown">
            <stop offset="0%" stopColor="#ccc" />
            <stop offset="100%" stopColor="#aaa" />
          </linearGradient>
          <linearGradient
            id="system-map-svg-gradient__planet-ring"
            gradientTransform="rotate(90)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet" gradientTransform="rotate(10)">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--gas-giant">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--gas-giant-ammonia">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--rocky">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--rocky-icy">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--icy">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--high-metal">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--metal-rich">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--earth-like">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <linearGradient id="system-map-svg-gradient__planet--landable">
            <stop offset="0%" />
            <stop offset="100%" />
          </linearGradient>
          <radialGradient id="system-map-svg-gradient__planet--atmosphere--radial">
            <stop offset="25%" />
            <stop offset="50%" />
            <stop offset="100%" />
          </radialGradient>
          <linearGradient
            id="system-map-svg-gradient__planet--atmosphere"
            gradientTransform="rotate(10)"
          >
            <stop offset="50%" />
            <stop offset="100%" />
          </linearGradient>

          {/* Filters */}
          <filter id="svg-filter__planet-shadow">
            <feOffset dx="-500" dy="-500" />
            <feGaussianBlur stdDeviation="200" result="offset-blur" />
            <feComposite operator="out" in="SourceGraphic" in2="offset-blur" result="inverse" />
            <feFlood floodColor="black" floodOpacity="1" result="color" />
            <feComposite operator="in" in="color" in2="inverse" result="shadow" />
            <feComponentTransfer in="shadow" result="shadow">
              <feFuncA type="linear" slope=".7" />
            </feComponentTransfer>
            <feComposite operator="over" in="shadow" in2="SourceGraphic" />
          </filter>
          <filter id="svg-filter__planet-shadow--small">
            <feOffset dx="-200" dy="-200" />
            <feGaussianBlur stdDeviation="100" result="offset-blur" />
            <feComposite operator="out" in="SourceGraphic" in2="offset-blur" result="inverse" />
            <feFlood floodColor="black" floodOpacity="1" result="color" />
            <feComposite operator="in" in="color" in2="inverse" result="shadow" />
            <feComponentTransfer in="shadow" result="shadow">
              <feFuncA type="linear" slope=".7" />
            </feComponentTransfer>
            <feComposite operator="over" in="shadow" in2="SourceGraphic" />
          </filter>
          <filter id="svg-filter__star-glow">
            <feMorphology id="morph" operator="dilate" radius="2000" />
            <feOffset dx="0" dy="0" />
            <feGaussianBlur stdDeviation="500" result="offset-blur" />
            <feComposite operator="out" in="SourceGraphic" in2="offset-blur" result="inverse" />
            <feFlood floodColor="rgba(255,0,0,.5)" floodOpacity="1" result="color" />
            <feComposite operator="in" in="color" in2="inverse" result="shadow" />
            <feComponentTransfer in="shadow" result="shadow">
              <feFuncA type="linear" slope="1" />
            </feComponentTransfer>
            <feComposite operator="over" in="shadow" in2="SourceGraphic" />
          </filter>
          <filter id="svg-filter__star-glow--light" x="-200%" y="-200%" width="500%" height="500%">
            <feMorphology id="morph" operator="dilate" radius="2000" />
            <feOffset dx="0" dy="0" />
            <feGaussianBlur stdDeviation="500" result="offset-blur" />
            <feComposite operator="out" in="SourceGraphic" in2="offset-blur" result="inverse" />
            <feFlood floodColor="rgba(255,0,0,.25)" floodOpacity="1" result="color" />
            <feComposite operator="in" in="color" in2="inverse" result="shadow" />
            <feComponentTransfer in="shadow" result="shadow">
              <feFuncA type="linear" slope="1" />
            </feComponentTransfer>
            <feComposite operator="over" in="shadow" in2="SourceGraphic" />
          </filter>

          {/* Animation */}
          <animate
            xlinkHref="#morph"
            id="anim-dialiate"
            attributeName="radius"
            from="40"
            to="0"
            dur="3s"
            fill="freeze"
          />
        </defs>
      </svg>

      <svg style={{ position: "absolute", height: 0, margin: 0, padding: 0, top: "-100px" }}>
        <defs>
          {/* Patterns */}
          <pattern
            id="svg-pattern__star-surface"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/star.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/rock.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface-animated"
            x="0"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/rock.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--clouds"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/clouds.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant-ammonia"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant-water"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant-sudarsky-class-1"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant-sudarsky-class-2"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant-sudarsky-class-3"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant-sudarsky-class-4"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--gas-giant-sudarsky-class-5"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/gas-giant.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--high-metal"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/rock.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--rocky-icy"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/rock.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--metal-rich"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/rock.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--earth-like"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/clouds.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
          <pattern
            id="svg-pattern__planet-surface--brown-dwarf"
            patternUnits="userSpaceOnUse"
            preserveAspectRatio="none"
            width="4096"
            height="4096"
          >
            <image href="/images/textures/star.jpg" x="0" y="0" width="4096" height="4096" />
            <animate attributeName="x" values="0;4096" dur="30s" repeatCount="indefinite" />
          </pattern>
        </defs>
      </svg>

      <style jsx>{`
        .system-map__system-object[data-system-object-type="Star"] .system-map__planet-surface {
          fill: url(#svg-pattern__star-surface) !important;
        }
        .system-map__system-object[data-system-object-type="Star"][data-system-object-sub-type*="Brown dwarf" i]
          .system-map__planet-surface {
          fill: url(#svg-pattern__planet-surface--brown-dwarf) !important;
        }
      `}</style>
    </>
  );
};

export default memo(SvgFilters);
